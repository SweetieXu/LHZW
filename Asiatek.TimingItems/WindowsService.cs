...
(SELECT SUBSTRING(ve.PlateNum, 2,LEN((ve.PlateNum))) FROM dbo.VehicleElectricFence vef INNER JOIN dbo.Vehicles ve ON ve.ID=vef.VehicleID ) 
 ORDER BY InOrOutTime ASC ", sTime, eTime);
  ...
  INNER JOIN dbo.Vehicles ve ON  ve.ID = te.LinkedVehicleID   
                                     WHERE efe.ExceptionType=0 AND (efe.SignalStartTime BETWEEN '{0}' AND '{1}' OR efe.SignalEndTime BETWEEN '{0}' AND '{1}' )  AND efe.SignalEndTime IS NOT NULL 
                                ORDER BY efe.SignalStartTime ASC ", sTime, eTime);
            ...
            //tempDt.Columns.Add(FontHelper.StringConvert("电子围栏异常信息", "1"), typeof(string));
            ...
            if (gateDt.Rows.Count == 0 && efDt.Rows.Count != 0)
            {
                //for (int i = 0; i < efDt.Rows.Count; i++)
                //{
                //    tempDt.Rows.Add(efDt.Rows[i]["PlateNum"], FontHelper.StringConvert("电子围栏异常", "1"), "", "", "", FontHelper.StringConvert("都不匹配", "1"), efDt.Rows[i]["SignalStartTime"], efDt.Rows[i]["SignalEndTime"]);
                //}
            }
            ...
            else
            {
                ...
                //    {
                //        tempDt.Rows.Add(efDt.Rows[i]["PlateNum"], FontHelper.StringConvert("电子围栏异常", "1"), "", "", "", FontHelper.StringConvert("开始时间不匹配", "1"), efDt.Rows[i]["SignalStartTime"], efDt.Rows[i]["SignalEndTime"]);
                //    }
                //    //电子围栏异常结束时间匹配不到门岗信息
                //    if (InCount > 0 && OutCount < 1)
                //    {
                //        tempDt.Rows.Add(efDt.Rows[i]["PlateNum"], FontHelper.StringConvert("电子围栏异常", "1"), "", "", "", FontHelper.StringConvert("结束时间不匹配", "1"), efDt.Rows[i]["SignalStartTime"], efDt.Rows[i]["SignalEndTime"]);
                //    }
