<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NGAreaOverSpeedRpt.aspx.cs" Inherits="Asiatek.TMS.ReportFiles.reports.NGAreaOverSpeedRpt" %>
...
<head id="Head1" runat="server">
    ...
    <script>
        ...
        var $hidStartTime;
        var $hidEndTime;
        var $hidAverageOfOverSpeed;
        var $hidActualDuration;

        var $txtVehicleName;
        var $txtStartTime;
        var $txtEndTime;
        var $txtAverageOfOverSpeed;
        var $txtActualDuration;
        ...
        var checkDateRange = function () {
            ...
            var r = /^[0-9]*[1-9][0-9]*$/;
            if (!(typeof ($("#txtAverageOfOverSpeed")) == "undefined" || $("#txtAverageOfOverSpeed").val() == null || $("#txtAverageOfOverSpeed").val() == "")) {
                if (!r.test($("#txtAverageOfOverSpeed").val())) {
                    $.showPromptDialog('<%=string.Format(Asiatek.Resource.DataAnnotations.PositiveIntegerError,Asiatek.Resource.DisplayText.AverageOfOverSpeed)%>', _title, _close);
                    return false
                } else if ($("#txtAverageOfOverSpeed").val() < 0) {
                    $.showPromptDialog('<%=string.Format(Asiatek.Resource.DataAnnotations.PositiveIntegerError,Asiatek.Resource.DisplayText.AverageOfOverSpeed)%>', _title, _close);
                        return false
                    }
            }
            if (!(typeof ($("#txtActualDuration")) == "undefined" || $("#txtActualDuration").val() == null || $("#txtActualDuration").val() == "")) {
                if (!r.test($("#txtActualDuration").val())) {
                    $.showPromptDialog('<%=string.Format(Asiatek.Resource.DataAnnotations.PositiveIntegerError,Asiatek.Resource.DisplayText.ActualDuration1)%>', _title, _close);
                    return false
                } else if ($("#txtActualDuration").val() < 0) {
                    $.showPromptDialog('<%=string.Format(Asiatek.Resource.DataAnnotations.PositiveIntegerError,Asiatek.Resource.DisplayText.ActualDuration1)%>', _title, _close);
                        return false
                    }
            }

            document.getElementById("hidAverageOfOverSpeed").value = document.getElementById("txtAverageOfOverSpeed").value;
            document.getElementById("hidActualDuration").value = document.getElementById("txtActualDuration").value;
            return true;
        }
        ...
        $(function () {
            ...
            $hidAverageOfOverSpeed = $("#hidAverageOfOverSpeed");
            $hidActualDuration = $("#hidActualDuration");


            $txtVehicleName = $("#txtVehicleName");
            $txtStartTime = $("#txtStartTime");
            $txtEndTime = $("#txtEndTime");
            $txtAverageOfOverSpeed = $("#txtverageOfOverSpeed");
            $txtActualDuration = $("#txtActualDuration");
        ...
        <div>
            <table>
                <tr>
                    ...
                    <td>&nbsp;&nbsp;&nbsp;</td>
                    <td>
                        <label><%=Asiatek.Resource.DisplayText.AverageOfOverSpeed %>：</label>
                    </td>
                    <td>
                        <input type="text" id="txtAverageOfOverSpeed"  name="txtAverageOfOverSpeed" class="form-control" placeholder="<%=Asiatek.Resource.DisplayText.AverageOfOverSpeedKM %>" />
                        <asp:HiddenField ID="hidAverageOfOverSpeed" runat="server" ClientIDMode="Static" />
                    </td>
                    <td>&nbsp;&nbsp;&nbsp;</td>
                    <td>
                        <label><%=Asiatek.Resource.DisplayText.ActualDuration1 %>：</label>
                    </td>
                    <td>
                        <input type="text" id="txtActualDuration"  name="txtActualDuration" class="form-control" placeholder="<%=Asiatek.Resource.DisplayText.ActualDuration %>" />
                        <asp:HiddenField ID="hidActualDuration" runat="server" Value="" ClientIDMode="Static" />
                    </td>
                    <td>&nbsp;&nbsp;&nbsp;</td>
                    <td>
