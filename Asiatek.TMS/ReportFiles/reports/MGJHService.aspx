<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MGJHService.aspx.cs" Inherits="Asiatek.TMS.ReportFiles.reports.MGJHService" %>
...
<head id="Head1" runat="server">
    ...
    <script>
        ...
        var checkDateRange = function () {
            ...
            var r = /^[0-9]*[1-9][0-9]*$/;
            if (!(typeof($("#txtTransportDuration"))=="undefined"||$("#txtTransportDuration").val()==null||$("#txtTransportDuration").val()=="")) {
                if (!r.test($("#txtTransportDuration").val())) {
                    $.showPromptDialog('<%=string.Format(Asiatek.Resource.DataAnnotations.PositiveIntegerError,Asiatek.Resource.DisplayText.TransportDuration)%>', _title, _close);
                    return false
                } else if ($("#txtTransportDuration").val() < 0) {
                    $.showPromptDialog('<%=string.Format(Asiatek.Resource.DataAnnotations.PositiveIntegerError,Asiatek.Resource.DisplayText.TransportDuration)%>', _title, _close);
                        return false
                    }
            }

            if (!(typeof ($("#txtStayDuration")) == "undefined" || $("#txtStayDuration").val() == null || $("#txtStayDuration").val() == "")) {
                if (!r.test($("#txtStayDuration").val())) {
                    $.showPromptDialog('<%=string.Format(Asiatek.Resource.DataAnnotations.PositiveIntegerError,Asiatek.Resource.DisplayText.StayDuration)%>', _title, _close);
                    return false
                } else if ($("#txtStayDuration").val() < 0) {
                    $.showPromptDialog('<%=string.Format(Asiatek.Resource.DataAnnotations.PositiveIntegerError,Asiatek.Resource.DisplayText.StayDuration)%>', _title, _close);
                        return false
                    }
            }
        ...
        $(function () {
            ...
            $hidTransportDuration = $("#hidTransportDuration");
            $hidStayDuration = $("#hidStayDuration");


            $txtVehicleName = $("#txtVehicleName");
            $txtStartTime = $("#txtStartTime");
            $txtEndTime = $("#txtEndTime");
            $txtTransportDuration = $("#txtTransportDuration");
            $txtStayDuration = $("#txtStayDuration");
        ...
        <div>
            <table>
                <tr>
                    ...
                    <td>&nbsp;&nbsp;&nbsp;</td>
                    <td>
                        <Label><%=Asiatek.Resource.DisplayText.ViewMode %></Label>：
                    </td>
                    <td>
                        <asp:DropDownList runat="server" ID="ddlViewMode"  CssClass="form-control">
                            <asp:ListItem Selected="True" Value="0">已结束</asp:ListItem>
                            <asp:ListItem Value="-1">全部</asp:ListItem>
                        </asp:DropDownList>
                    </td>
                    <td>&nbsp;&nbsp;&nbsp;</td>
                    <td>
                        <label><%=Asiatek.Resource.DisplayText.TransportDuration %>：</label>
                    </td>
                    <td>
                        <input type="text" id="txtTransportDuration" style="width:150px" runat="server" name="txtTransportDuration" class="form-control" />
                    </td>
                    <td>&nbsp;&nbsp;&nbsp;</td>
                    <td>
                        <label><%=Asiatek.Resource.DisplayText.StayDuration %>：</label>
                    </td>
                    <td>
                        <input type="text" id="txtStayDuration" runat="server" style="width:150px" name="txtStayDuration" class="form-control" />
                    </td>
                    <td>&nbsp;&nbsp;&nbsp;</td>
                    <td>
