...
(function ($) {
    ...
    jQuery.playBar = {
        addBar: function (DOM, allTime) {
            ...
            alltime = allTime;
            DOM.empty();
            DOM.append("<div class='BarControlPlay'><div style='visibility:hidden;' id='BarBeginTime' class='BarBeginTime'>00:00</div></div>");
            $(".BarControlPlay").append('<div class="TheBar"><div id="TimeBall" class="TimeBall"></div><div class="TheColorBar"></div></div><div id="BarFinishTime" style="visibility:hidden;" class="BarFinishTime">10:35</div>');
            width = $('.TheBar').width();
            times = allTime / 1000;
            rwidth = width - 8;
...
/******处理拖动事件******/
    ...
    var lastX = 0, NewX = 0;//记录前后位置click
    ...
    //$(document).on("mouseout", '.TimeBall', function (event) {
    //    lastX = event.clientX;
    //    event.preventDefault();
    //    down = false;
    //    BarMove = false;
    //    if (true) {
    //        StopBar();//停止滑动mouseout
    //    }
    //});//鼠标按下
