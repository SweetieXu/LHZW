<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TemperForJSYY.aspx.cs" Inherits="Asiatek.TMS.ReportFiles.reports.TemperForJSYY" %>
...
<head id="Head1" runat="server">
    ...
    <script>
        ...
        var checkDateRange = function () {
            ...
            if ($("#hidVehicleID").val() == "")
            {
                $.showPromptDialog('<%=Asiatek.Resource.DataAnnotations.VehicleMustCheck%>', _title, _close);
                return false
            }
            var r = /^[0-9]*[1-9][0-9]*$/
            if (!r.test($("#txtTimespan").val()))
            {
                $.showPromptDialog('<%=Asiatek.Resource.DataAnnotations.TimeSpanNum%>', _title, _close);
                return false
            } else if ($("#txtTimespan").val() < 2) {
                $.showPromptDialog('<%=Asiatek.Resource.DataAnnotations.TimespanMust%>', _title, _close);
                return false
            }
            return true;
        }
        ...
        <div>
            <table>
                <tr>
                    ...
                    <td><label><%=Asiatek.Resource.DisplayText.SignalDateTimeSpan %>:</label></td>
                    <td>
                        <input type="text" id="txtTimespan" runat="server" value="2" class="form-control"/>
                    </td>
                    <td>
